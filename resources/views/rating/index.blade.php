                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap" cellspacing="0"
                                        ...
                                        <tbody>
                                            @foreach ($rating as $key => $rating)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    <td>{{ Ucfirst($rating->doctor->name) }}</td>
                                                    <td>{{ $rating->total_rating }}</td>
                                                    <td>{{ $rating->user_name }}</td>

                                                    <td>
                                                        @if ($rating->status == '0')
                                                            <a href="{{ url('block', $rating->id) }}"
                                                                class="btn btn-danger btn-xs">Approve</a>

                                                        @else
                                                            <a href="{{ url('unblock', $rating->id) }}"
                                                                class="btn btn-success btn-xs">UnApprove</a>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <button type="button" class="edit-btn btn btn-danger"
                                                            ...
                                                            description="{{ $rating->description }}" data-toggle="modal"
                ...
                <div class="modal-content">
                    ...
                    <form id="updateForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group row">
                                <label for="example-text-input" class="col-sm-12 col-form-label"><strong>User Comment
                                </label>
                                <div class="col-sm-12 ">
